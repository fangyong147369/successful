...
import com.zc.common.dao.BaseDao;
import com.zc.common.model.jpa.Page;
import com.zc.common.model.jpa.PageDataList;
import com.zc.common.model.jpa.QueryParam;
import com.zc.common.util.log.LogUtil;
import com.zc.common.util.reflect.ReflectUtil;
...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	@Override
	public <T> PageDataList<T> findPageListBySQL(String querySQL, String formSql, QueryParam param, Class<T> clazz) {
		if (formSql.indexOf("where") < 0) {
			formSql = formSql + " where 1=1 ";
		}
		String countSql = "select count(1) " + formSql + param.bySearchSqlFilter().toString();
		...
		param.addPage(count, param.getPage().getCurrentPage(), param.getPage().getPernum());
		String selectSQL = querySQL + formSql + param.bySearchSqlFilter().toString()
				+ param.byOrderSqlFilter().toString();
